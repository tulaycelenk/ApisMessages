...
namespace ApisMessages
{
    class Version15B:ApisEdifactHelper
    {
        ...
        //ÜLKEYE ALINDIĞINDA TANIMI YAPILMALI
        ...
        public byte[] Export(FlightResponse flight, string exporterType, bool singleOrMultiPaxLst)
            {
             ...
             IEnumerable<Passenger> passengerList = PassengerService.QueryPassengersJoine(new PassengerPagingRequest { FlightId = flight.Id }).Where(p => p.PaxStatusCode == ParameterHelper.PaxStatusCode.Flown || p.PaxStatusCode == ParameterHelper.PaxStatusCode.Boarded || p.PaxStatusCode == ParameterHelper.PaxStatusCode.CheckedIn).OrderBy(s => s.Surname);
            ...
            apisMessageHeaderList.Add(unh + HTcode + unhMid1 + version15B + unhMid2);
            ...
            foreach (var passengerDocs in passengerDocsList)
            {
                ...
                apisMessagePassengerList.Add(loc91 + threeColon + CITY CODE + apos);
