...
{
    class Version12B : ApisEdifactHelper
    {
        public byte[] Export(FlightResponse flight, string exporterType, bool singleOrMultiPaxLst)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                if (!singleOrMultiPaxLst)
                {
                    ...
                    tw.WriteLine(una);
                    tw.WriteLine(unb + sender+ airlineName + yearToMin + plus + BZcode + unbEnd);
                    tw.WriteLine(ung + airlineName + yearToMin + plus + GEcode + typeVersionNum + version12B);
                    tw.WriteLine(unh + HTcode + unhMid1 + version12B + unhMid2);
                    tw.WriteLine(bgm745);
                    tw.WriteLine(nadMs);
                    tw.WriteLine(com);
                            ...
                            for (var i = 0; i < passengerBaggages.Count(); i++)
                            {
                                ...
                                else
                                {
                                    if (isSequentialBagTag)
                                        tw.WriteLine(sequentialBagTagText);
                                    else
                                        tw.WriteLine(ftx+flight.AirlineIataCode+passengerBaggages.ElementAt(i).BagTag +apos);
